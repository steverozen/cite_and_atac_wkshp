---
title: "CITE-seq / multimodal tutorial"
format: html
---

# From: https://satijalab.org/seurat/articles/multimodal_vignette
...
#| eval: false
# Just download once
# Change to eval: true if you need to download

curl_download("https://github.com/steverozen/cite_and_atac_wkshp/raw/refs/heads/main/GSE100866_CBMC_8K_13AB_10X-ADT_umi.csv.gz",
destfile = "GSE100866_CBMC_8K_13AB_10X-ADT_umi.csv.g")

curl_download("https://github.com/steverozen/cite_and_atac_wkshp/raw/refs/heads/main/GSE100866_CBMC_8K_13AB_10X-RNA_umi.csv.gz",
desftile = "GSE100866_CBMC_8K_13AB_10X-RNA_umi.csv.gz"")

```

This next cell might take a minute or 2
...
```{r}
# Normalize ADT data,
DefaultAssay(cbmc) <- "ADT" # Antibody derived tag
...
# I.e. we prepend the key, such as "adt_", to the feature identifier, for example "CD19".
...
download.file(
  ...
  destfile = "pbmc_10k_protein_v3_filtered_feature_bc_matrix.tar.gz"
)
```

```{r}
untar("pbmc_10k_protein_v3_filtered_feature_bc_matrix.tar.gz")
```
